...
ESX.RegisterServerCallback("suku:getShopItems", function(source, cb, shoptype)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "casino" then
			for _, v in pairs(Config.Shops.Casino.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
		...
		if shoptype == "hospital" then
			for _, v in pairs(Config.Shops.Hospital.Items) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
						type = "item_standard",
						name = itemInformation[itemResult[i].name].name,
...
ESX.RegisterServerCallback("suku:getCustomShopItems", function(source, cb, shoptype, customInventory)
	...
	for i=1, #itemResult, 1 do
		...
		if shoptype == "weaponshopPolice" then
			local weapons = customInventory.Weapons
			for _, v in pairs(customInventory.Weapons) do
				if v.name == itemResult[i].name then
					table.insert(itemShopList, {
        ...
        --if targetItem.weight == -1 or ((targetItem.count + count) <= targetItem.weight) then
            ...
            for i = 1, #list, 1 do
				if list[i].name == item then
					...
					TriggerClientEvent('b1g_notify:client:Notify', source, { type = 'true', text = 'Vásároltál:  '..count.." "..list[i].label})
					else
				TriggerClientEvent('b1g_notify:client:Notify', source, { type = 'false', text = _U('no_money')})
					end
				end
...
RegisterNetEvent("suku:SellItemToCasino")
AddEventHandler("suku:SellItemToCasino",function(source, type, item, count)
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)

    if type == "item_standard" and item == 'cchip' then
		local targetItem = xPlayer.getInventoryItem(item)
		if xPlayer.canCarryItem(item, count) then
        --if targetItem.weight == -1 or ((targetItem.count + count) <= targetItem.weight) then
            local list = itemShopList
            for i = 1, #list, 1 do
				if list[i].name == item then
					local totalPrice = count * list[i].price
					if count <= tonumber(targetItem.count) then
						xPlayer.removeInventoryItem(item, count)
						xPlayer.addMoney(totalPrice)
					TriggerClientEvent('b1g_notify:client:Notify', source, { type = 'true', text = 'Vásároltál:  '..count.." "..list[i].label})
					else
				TriggerClientEvent('b1g_notify:client:Notify', source, { type = 'false', text = _U('invalid_quantity')})
					end
				end
            end
        else

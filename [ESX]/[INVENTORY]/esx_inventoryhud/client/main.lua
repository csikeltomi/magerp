...
RegisterNetEvent("esx_inventoryhud:ForceOpenInventory")
AddEventHandler("esx_inventoryhud:ForceOpenInventory", function ()
    openInventory()
end)
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        if type(data.number) == "number" and math.floor(data.number) == data.number then
            ...
            elseif string.match(data.item.name,'_key_') then
                TriggerServerEvent("esx_property:sendProperties")
...
function loadPlayerInventory()
    ESX.TriggerServerCallback(
        ...
        function(data)
            ...
            if inventory ~= nil then
                for key, value in pairs(inventory) do
                    if inventory[key].count <= 0 then
                        ...
                        if Split(inventory[key].name,'_')[1] == 'm' then
                            inventory[key].usable = false
                            inventory[key].canRemove = false
                            table.insert(items2, inventory[key])
                        else
            ...
            SendNUIMessage(
                {
                    action = "setMageInventoryItems",
                    itemList = items2
                }
            )
            TriggerServerEvent('Magic:LoadQB')
        end,
        GetPlayerServerId(PlayerId())
    )
end

RegisterNetEvent("InventoryHud:InventoryUpdate")
AddEventHandler("InventoryHud:InventoryUpdate",function()
    while ESX == nil do
        Citizen.Wait(1)
    end
    loadPlayerInventory()
...
RegisterNetEvent("InventoryHud:LoadQB")
AddEventHandler('InventoryHud:LoadQB',function(qb)
    SendNUIMessage(
                {
                    action = "setQbItems",
                    itemList = qb

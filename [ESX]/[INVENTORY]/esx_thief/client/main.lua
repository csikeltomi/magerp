                    ...
                    IsAbleToSteal(target_id, function(err)
                        ...
                        if not err  then
                            ESX.TriggerServerCallback('esx_thief:getItemQ', function(quantity)
                                if quantity > 0 then
                                    IsAbleToSearch = true
                                    TriggerServerEvent('esx_ruski_areszt:startAreszt',GetPlayerServerId(player)) -- Rozpoczyna Funkcje na Animacje (Cala Funkcja jest Powyzej^^^)
						            Citizen.Wait(3000)
...
RegisterCommand('cuffmenu', function(source, args, rawCommand)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
            ...
            if IsControlJustPressed(1, Keys['H']) and not IsEntityDead(ped) and not IsPedInAnyVehicle(ped, true) then -- OPEN CUFF MENU
                OpenCuffMenu()
            end
    end
end)

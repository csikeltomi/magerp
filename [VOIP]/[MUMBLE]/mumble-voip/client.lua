...
Citizen.CreateThread(function ()
	Wait(10000)
	SetCallChannel(123)
	print("MYID " .. GetPlayerServerId(PlayerId()))
end)
...
--[[Citizen.CreateThread(function()
	...
	while true do
		ESX.TriggerServerCallback("mumble-voip:HasItem", function(cb)
			hasitem = cb
		end,'hangosbeszelo')
		Citizen.Wait(1000)
	end
			...
			if IsControlJustPressed(0, mumbleConfig.controls.proximity.key) then
				...
				if not secondaryPressed then
					...
					if newMode > #mumbleConfig.voiceModes then
						...
						if(newMode == 4 and IsPedInAnyVehicle(playerPed)) then
							...
							if GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "POLICE" then -- BLACKLISTED VEHICLE
								...
								elseif GetDisplayNameFromVehicleModel(GetEntityModel(veh)) == "430" then -- BLACKLISTED VEHICLE
							end

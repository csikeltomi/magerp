...
Citizen.CreateThread(function()
	for i = 0, 128 do SendVoiceToPlayer(i, false) end
	...
	while true do
		...
		if sendVoice then
			if not shouldReset then
				shouldReset = true
				TriggerEvent("Makez:Speaker",Voice.Mode,true) -- you can implement a microphone icon
			end
		elseif not sendVoice and shouldReset then
			shouldReset = false
			TriggerEvent("Makez:Speaker",Voice.Mode,false)
			for i = 0, 128 do

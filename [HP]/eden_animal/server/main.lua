...
ESX.RegisterServerCallback('eden_animal:getAnimals', function (source, cb)
	local result = {}

	exports.discord_perms:IsRolePresent(source,"Prémium", function(hasRole)
		if hasRole then
			cb(Config.PetShop)
			print("HASROLE")
		else
			print("filter")
			for i=1, #Config.PetShop, 1 do
				if Config.PetShop[i].premium == false then
					table.insert(result,Config.PetShop[i])
				end
			end
			cb(result)
		end

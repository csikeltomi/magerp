...
AddEventHandler('esx:addInventoryItem', function(item, count, showNotification)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
			...
			local splitted = Split(item,'_')
			if not splitted[1] or (splitted[1] and splitted[1] ~= 'm') then
			exports['okokNotify']:Alert("TÁROLÓ", "Kaptál " .. (count - v.count) .. " x" .. v.label .. ' tárgyat', 5000, 'info')
			end
...
AddEventHandler('esx:removeInventoryItem', function(item, count, showNotification)
	for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
			exports['okokNotify']:Alert("TÁROLÓ", (v.count - count) .. " x" .. v.label .. ' tárgy eltávolítva', 5000, 'info')
 ...
 RegisterNetEvent('esx:removeInventoryItemSilent')
    AddEventHandler('esx:removeInventoryItemSilent', function(item, count)
      for k,v in ipairs(ESX.PlayerData.inventory) do
		if v.name == item then
			--ESX.UI.ShowInventoryItemNotification(false, v.label, v.count - count)
			ESX.PlayerData.inventory[k].count = count
...
AddEventHandler('esx:createPickup', function(pickupId, label, coords, type, name, components, tintIndex)
	...
	if type == 'item_weapon' then
		...
		local model = GetHashKey('v_res_m_spanishbox')
		RequestModel(model)
		while not HasModelLoaded(model) do
			Citizen.Wait(1)
		end
		ESX.Game.SpawnLocalObject('v_res_m_spanishbox', coords, setObjectProperties)
	end
end)

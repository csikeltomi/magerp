	...
	table.insert(tasks, function(cb)
		MySQL.Async.fetchAll('SELECT accounts, job, job_grade, `group`, loadout, position, inventory FROM users WHERE identifier = @identifier', {
			...
			if result[1].position and result[1].position ~= '' then
				...
				userData.coords = {x = -1034.4361572266, y = -2778.0646972656, z = 4.6397986412048, heading = 6.0}
			end
...
AddEventHandler('esx:giveInventoryItem', function(target, type, itemName, itemCount)
	...
	if type == 'item_standard' then
		...
		if itemCount > 0 and sourceItem.count >= itemCount then
			if targetXPlayer.canCarryItem(itemName, itemCount) then
				...
				sourceXPlayer.showNotification(_U('gave_item', itemCount, sourceItem.label, target))
				targetXPlayer.showNotification(_U('received_item', itemCount, sourceItem.label, playerId))
			else
				sourceXPlayer.showNotification(_U('ex_inv_lim', target))
			end
		else
	...
	elseif type == 'item_account' then
		if itemCount > 0 and sourceXPlayer.getAccount(itemName).money >= itemCount then
			...
			sourceXPlayer.showNotification(_U('gave_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], target))
			targetXPlayer.showNotification(_U('received_account_money', ESX.Math.GroupDigits(itemCount), Config.Accounts[itemName], playerId))
		else
		...
		if sourceXPlayer.hasWeapon(itemName) then
			...
			if not targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo and itemCount > 0 then
					...
					sourceXPlayer.showNotification(_U('gave_weapon_withammo', weaponLabel, itemCount, ammoLabel, target))
					targetXPlayer.showNotification(_U('received_weapon_withammo', weaponLabel, itemCount, ammoLabel, playerId))
				else
					sourceXPlayer.showNotification(_U('gave_weapon', weaponLabel, target))
					targetXPlayer.showNotification(_U('received_weapon', weaponLabel, playerId))
				end
			else
				sourceXPlayer.showNotification(_U('gave_weapon_hasalready', target, weaponLabel))
				targetXPlayer.showNotification(_U('received_weapon_hasalready', playerId, weaponLabel))
			end
		end
			...
			if targetXPlayer.hasWeapon(itemName) then
				...
				if weaponObject.ammo then
					...
					if weapon.ammo >= itemCount then
						...
						sourceXPlayer.showNotification(_U('gave_weapon_ammo', itemCount, ammoLabel, weapon.label, target))
						targetXPlayer.showNotification(_U('received_weapon_ammo', itemCount, ammoLabel, weapon.label, playerId))
					end
				end
			else
				sourceXPlayer.showNotification(_U('gave_weapon_noweapon', target))
				targetXPlayer.showNotification(_U('received_weapon_noweapon', playerId, weapon.label))
			end
		end
	end
end)
...
RegisterNetEvent('esx:removeDropInventoryItem')
AddEventHandler('esx:removeDropInventoryItem', function(type, itemName, itemCount)
	local playerId = source
	local xPlayer = ESX.GetPlayerFromId(source)
	...
	if type == 'item_standard' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > xItem.count or xItem.count < 1) then
				...
				xPlayer.removeInventoryItem(itemName, itemCount)
				local pickupLabel = ('~y~%s~s~ [~b~%s~s~]'):format(xItem.label, itemCount)
				ESX.CreateRandomPickup('item_standard', itemName, itemCount, pickupLabel, playerId)
				xPlayer.showNotification(_U('threw_standard', itemCount, xItem.label))
			end
		end
	elseif type == 'item_account' then
		if itemCount == nil or itemCount < 1 then
			...
			if (itemCount > account.money or account.money < 1) then
				...
				xPlayer.removeAccountMoney(itemName, itemCount)
				local pickupLabel = ('~y~%s~s~ [~g~%s~s~]'):format(account.label, _U('locale_currency', ESX.Math.GroupDigits(itemCount)))
				ESX.CreateRandomPickup('item_account', itemName, itemCount, pickupLabel, playerId)
				xPlayer.showNotification(_U('threw_account', ESX.Math.GroupDigits(itemCount), string.lower(account.label)))
			end
		end
	elseif type == 'item_weapon' then
		...
		if xPlayer.hasWeapon(itemName) then
			...
			ESX.CreateRandomPickup('item_weapon', itemName, weapon.ammo, pickupLabel, playerId, components, weapon.tintIndex)
		end
	end
end)
	...
	if pickup then
		if pickup.type == 'item_standard' then
			if xPlayer.canCarryItem(pickup.name, pickup.count) then
				local _mage = Split(pickup.name,'_')
				if _mage ~= nil then
					if _mage[1] =='m' then
						local inventory = xPlayer.getInventory(true)
						local mage = 'm_' .. _mage[2] .. '_' .. _mage[3]
						for k,v in pairs(inventory) do
							if string.match(k,mage) then
								xPlayer.showNotification(_U('you_have_this_magic'))
								success = false
								return
							end
						end

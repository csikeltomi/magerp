...
ESX.CreateRandomPickup = function(type, name, count, label, playerId, components, tintIndex)
	local pickupId = (ESX.PickupId == 65635 and 0 or ESX.PickupId + 1)
	local xPlayer = ESX.GetPlayerFromId(playerId)
	local coords = xPlayer.getCoords()
	coords = vector3(coords.x + RandomDistance(), coords.y + RandomDistance(), coords.z)
	ESX.Pickups[pickupId] = {
		type = type, name = name,
		count = count, label = label,
		coords = coords
...
ESX.CreateLootPickup = function(type, name, count, label, coords)
	local pickupId = (ESX.PickupId == 65635 and 0 or ESX.PickupId + 1)
	ESX.Pickups[pickupId] = {
		type = type, name = name,
		count = count, label = label,
	...
	local pickupLabel = ('~y~%s~s~ [~b~%s~s~]'):format(label, count)
	if type == 'item_weapon' then
		ESX.Pickups[pickupId].components = nil
		ESX.Pickups[pickupId].tintIndex = nil
	end

	TriggerClientEvent('esx:createPickup', -1, pickupId, pickupLabel, coords, type, name, nil, nil)
	ESX.PickupId = pickupId
end

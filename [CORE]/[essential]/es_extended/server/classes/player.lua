function CreateExtendedPlayer(playerId, identifier, group, accounts, inventory, weight, job, loadout, name, coords)
	...
	self.removeInventoryItemSilent = function(name, count)
		local item = self.getInventoryItem(name)
		...
		if item then
			...
			if newCount >= 0 then
				...
				TriggerEvent('esx:onRemoveInventoryItem', self.source, item.name, item.count)
				self.triggerEvent('esx:removeInventoryItemSilent', item.name, item.count)
			end
		end
	end
	...
	self.setInventoryUsage = function(name, usage)
		local item = self.getInventoryItem(name)

		if item then
			local newusage = item.used + usage
			item.used = newusage
			if newusage <= 0 then
				self.removeInventoryItem(item.name, 1)
			end
		end
	end

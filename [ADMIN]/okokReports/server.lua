	...
	if staffs[source] then
		if canAssist then
			...
			if GetPlayerPing(id) > 0 then
				...
				TriggerClientEvent('okokNotify:Alert', source, "REPORT", "Következőben segítesz: BEJELENTÉS #"..feedbackId.."!", 20000, 'info')
				TriggerClientEvent('okokNotify:Alert', id, "REPORT", "Egy admin érkezett!", 20000, 'info')

				if Webhook ~= '' then
					assistFeedbackWebhook(assistFeedback)
				end
			else	
				TriggerClientEvent('okokNotify:Alert', id, "REPORT", "A játékos már nincs a szerveren!", 20000, 'error')
			end

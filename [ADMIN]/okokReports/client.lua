...
RegisterCommand(Config.FeedbackClientCommand, function(source, args, rawCommand)
	if canFeedback then
		...
		exports['okokNotify']:Alert("REPORT", "Most jelentettél valamit. Várakozz egy adminra!", 10000, 'warning')
	end
end, false)
...
exports['okokNotify']:Alert("REPORT", "Új jelentés, valakinek segítségre van szüksége!", 20000, 'success')
		SendNUIMessage({
			action = "updateFeedback",
			FeedbackTable = FeedbackTable
		})
...
RegisterNUICallback("action", function(data)
	...
	if data.action == "newFeedback" then
		exports['okokNotify']:Alert("REPORT", "A bejelentést elküldtük a vezetőségnek!", 20000, 'success')
		...
		if FeedbackTable[data.feedbackid] then
			if oneSync then
				...
				if playerOnline then
					...
					exports['okokNotify']:Alert("REPORT", "Már nem tartózkodik a szerveren!", 20000, 'error')
				end
			end
		end
	elseif data.action == "concludeFeedback" then
		...
		if feedbackInfo then
			if feedbackInfo.concluded ~= true or canConclude then
				...
				exports['okokNotify']:Alert("FEEDBACK", "Jelentés #"..feedbackID.." lezárva!", 20000, 'success')
			end
		end
	end
end)
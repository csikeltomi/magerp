...
function reloadFeedbackTable() {
	...
	$.each(FeedbackTable, function(id, feedback) {
		...
		if (feedback.category === 'player_report') {
			category = "<span><i class='fas fa-user-alt'></i> SZABÁLYSZEGÉS</span>"
		} else if (feedback.category === 'question') {
			category = "<span><i class='fas fa-question'></i> KÉRDÉS</span>"
		} else {
			category = "<span><i class='fas fa-bug'></i> HIBA</span>"
		}
		...
		if (feedback.concluded != true) {
			feedbackTable.append(`
				<tr class="table-row" feedbackID="${id}">
					<td>VISSZAJELZÉS #${feedback.feedbackid}</th>
					<td>${category}</td>
					<td style="color: ${statusColor};">${concluded}</td>
					<td><button type="button" class="btn btn-view"><i class="fas fa-eye"></i> MEGTEKINTÉS</button></td>
				</tr>`
			)
		}
	});
	...
	if ($(".feedbackTable .table").children().length <= 0) {
		$(".feedbackTable .feedback_subtitle").html("SZÉP MUNKA, MOST MÁR PIHENHETSZ.");
		...
		$(".feedback_subtitle").html("HALI! VAN ITT EGY KIS MELÓ...");

...
AddEventHandler('trew_hud_ui:setInfo', function(info)
	...
	SendNUIMessage({ action = 'setText', id = 'jobGrade', value = info['grade'] })
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if IsControlJustPressed(1, Keys['B']) and GetPedInVehicleSeat(vehicle, -1) == player and (has_value(vehiclesCars, vehicleClass) == true) then
...
AddEventHandler('esx:onPlayerSpawn', function()
	...
	SendNUIMessage({ action = 'setText', id = 'id', value = GetPlayerServerId(PlayerId()) })
	if Config.ui.showVoice == true then
		if Config.voice.levels.current == 0 then
			NetworkSetTalkerProximity(Config.voice.levels.default)
		elseif Config.voice.levels.current == 1 then

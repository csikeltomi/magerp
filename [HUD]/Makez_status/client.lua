...
Citizen.CreateThread(function()
  while true do
    ...
    TriggerEvent('esx_status:getStatus', 'hunger', function(_status)
			status[0] = math.floor(_status.getPercent())
		end)
    TriggerEvent('esx_status:getStatus', 'thirst', function(_status)
			status[1] = math.floor(_status.getPercent())
		end)
    SendNUIMessage({
      action = "updateStatus",
      st = status,
...
RegisterNetEvent("Makez:Speaker")
AddEventHandler("Makez:Speaker", function(mode,talk)
  if mode == 1 then
    SendNUIMessage({voiceheal = 25});
  elseif mode == 2 then
    SendNUIMessage({voiceheal = 50});

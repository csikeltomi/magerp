...
function OpenActionMenuInteraction(target)
	...
	table.insert(elements, {label = ('Hordozás'), value = 'drag'})
	table.insert(elements, {label = ('Lovaglás'), value = 'piggyback'})
	table.insert(elements, {label = ('Cipelés'), value = 'carry'})
	table.insert(elements, {label = ('Túszul ejtés'), value = 'takehostage'})
	ESX.UI.Menu.CloseAll()

	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'action_menu',
		{
			title    = ('Hordozási típusok'),
			align    = 'top-left',
			elements = elements
		},function(data, menu)
			...
			if data.current.value == 'drag' then			
				...
				ESX.ShowNotification('Valakit probálsz felemelni...')
				...
				if hasRope == true then
					...
					if distance ~= -1 and distance <= 3.0 then
						...
						ESX.ShowNotification("Nincs senki a közeledben...")
					end
				else
			...
			if data.current.value == 'piggyback' then
				...
				if closestPlayer ~= nil then
					...
					print("[CMG Anim] Nincs senki a közeledben...")
	...
	for i=1, #hostageAllowedWeapons do
		...
		if closestPlayer ~= nil then
			...
			print("[CMG Anim] Nincs senki a közeledben...")
			drawNativeNotification("Nincs a közeledben senki!")
		end 
	end
	end
...
Citizen.CreateThread(function()
	while true do 
		if holdingHostage then
			...
			DrawText3D(playerCoords.x,playerCoords.y,playerCoords.z,"Nyomj [G]-t a szabadon engedéshez, [H]-t a megöléshez")

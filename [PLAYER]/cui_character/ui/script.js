...
function loadTabContent(tabName, charData, clothesData, identityData) {
    $.get('pages/' + tabName + '.html', function (data) {
        ...
        tab.html(data);
        if (tabName == 'style') {
            loadOptionalStyle(tab, charData, clothesData);
...
function loadOptionalStyle(element, charData, clothesData) {
    ...
    $.get(hairpage, function (data) {
        ...
        let hairlist = hair.find('select#hair_1.hairstyle');
        let nohair = 0;
        hairlist.empty();
        hairlist.append($('<option value="' + nohair + '" data-texture="0">None</option>')) // "None" option
        clothesData.hairlist.forEach(function (item) {
            let option = $('<option value="' + item.drawable + '"></option>');
            option.text(item.drawable)
...
function loadClothesTab(tab, clothesData, male) {
    ...
    let handslist = tab.find('select#hands.clotheslist');
    let nohand = male ? 15 : 15;
    handslist.empty();
    handslist.append($('<option data-component="3" data-drawable="' + nohand + '" data-texture="0">None</option>')) // "None" option
    clothesData.handslist.forEach(function (item) {
        let option = $('<option data-component="' + item.component + '" data-drawable="' + item.drawable + '" data-texture="' + item.texture + '"></option>');
        option.text(item.drawable)
        handslist.append(option);
    });
...
function GoNext(evt, num) {
    console.log("DO")
    let list = $(evt).siblings('select').first();
    ...
    if (list.prop('selectedIndex') == numOpt - 1) {
        ...
        newVal = $(list).find('option:contains(' + num + ')').first();
        if (newVal == null) {
...
function GoNext2(evt) {
    console.log("DO")
    let slider = $(evt).siblings('input[type=range]').first();
    let min = parseInt(slider.prop('min'));
    let max = parseInt(slider.prop('max'));
    let val = parseInt(slider.val());

    if ($(evt).hasClass('left')) {
        if (val > min) {
            slider.val(val - 1);
        }
    }
    else if ($(evt).hasClass('right')) {
        if (val < max) {
            slider.val(val + 1);
        }
    }
...
function refreshContentData(element, data) {
    ...
    */
    if (element.is('#apparel')) {
        ...
        refreshComponentList(element, '#hands', 8, data.arms, data.arms_2);

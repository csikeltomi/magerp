...
if not Config.StandAlone then  
    ...
    if Config.EnableESXIdentityIntegration then
        ESX.RegisterServerCallback('cui_character:updateIdentity', function(source, cb, _data)
            local xPlayer = ESX.GetPlayerFromId(source)
            local data = _data
            if xPlayer then
                MySQL.Async.fetchAll('SELECT * FROM `users` WHERE firstname = @firstName AND lastName = @lastName', {
                    ...
                    if #result == 0 then
                        if checkNameFormat(data.firstname) and checkNameFormat(data.lastname) and checkSexFormat(data.sex) and checkDOBFormat(data.dateofbirth) and checkHeightFormat(data.height) then
                            ...
                            xPlayer.addInventoryItem('m_0_INVITO_5', 1)
                            xPlayer.addInventoryItem('m_10_FIRE_4', 1)
        
                            saveIdentityToDatabase(xPlayer.identifier, currentIdentity)
                            TriggerClientEvent("Group:CastSelector", source)
                            cb(true)
                        else
                            cb(false)
                        end
